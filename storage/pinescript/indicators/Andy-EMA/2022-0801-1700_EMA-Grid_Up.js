...
//##### Destination
indicator(title="Test Over-下-2", shorttitle="Test Over-下-2", overlay=false, precision=8)
...
transperency = 60
maColor(ma, maRef) =>
ta.change(ma) >=0 and ma05 > ma100 ? color.new(colorLightGreen, transperency)
    : ta.change(ma) <0  and ma05 > ma100 ? color.new(colorDarkRed, transperency)
    : ta.change(ma)<=0 and ma05 < ma100 ? color.new(colorLightRed, transperency)
    : ta.change(ma) >=0 and ma05 < ma100 ? color.new(colorDarkGreen, transperency)
    //: color.new(colorGray, transperency)
    : ma05 > ma100 ? color.new(colorDarkRed, transperency) : color.new(colorDarkGreen, transperency)
...
// ##### Drawing
plot( 05, histbase=00, color=maColor(ma05, ma100), style=plot.style_columns, title="MMA05", linewidth=1, editable=false)
